...
export function ChatPopup({ currentUserId }: { currentUserId: string }) {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-4 right-4 z-50 flex h-[480px] w-[360px] flex-col overflow-hidden rounded-2xl border border-themed-subtle bg-card shadow-2xl">
          <div className="flex items-center justify-between border-b border-themed-subtle px-4 py-3">
            {selectedConvo ? (<><button onClick={() => setSelectedConvo(null)} className="rounded-lg p-1 text-dim hover:bg-hover"><ArrowLeft className="h-4 w-4" /></button><p className="text-sm font-semibold text-heading">{getOther(selectedConvo)?.name || getOther(selectedConvo)?.email || "Chat"}</p><button onClick={() => setOpen(false)} className="rounded-lg p-1 text-dim hover:bg-hover"><X className="h-4 w-4" /></button></>) : (<><p className="text-sm font-semibold text-heading">Messages</p><div className="flex items-center gap-1"><button onClick={handleNewChat} className="rounded-lg p-1.5 text-dim hover:bg-hover hover:accent-color"><Plus className="h-4 w-4" /></button><button onClick={() => setOpen(false)} className="rounded-lg p-1.5 text-dim hover:bg-hover"><X className="h-4 w-4" /></button></div></>)}
          </div>
          {!selectedConvo ? (
            <div className="flex-1 overflow-y-auto">{showNewChat ? (<div className="p-3">{teammates.map((t) => (<button key={t.id} onClick={() => startConvo(t.id)} className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-heading hover:bg-page"><div className="flex h-8 w-8 items-center justify-center rounded-full accent-bg text-xs font-semibold accent-color">{(t.name||t.email).charAt(0).toUpperCase()}</div><span className="truncate">{t.name||t.email}</span></button>))}<button onClick={() => setShowNewChat(false)} className="mt-2 w-full text-center text-xs text-body">Cancel</button></div>) : loading ? (<div className="flex items-center justify-center py-12"><div className="h-5 w-5 animate-spin rounded-full border-2 border-themed border-t-[var(--accent)]" /></div>) : conversations.length === 0 ? (<div className="flex flex-col items-center py-12"><MessageSquare className="h-8 w-8 text-dim mb-2" /><p className="text-sm text-body">No conversations</p><button onClick={handleNewChat} className="mt-2 text-sm font-medium accent-color">Start one</button></div>) : (conversations.map((c) => { const o = getOther(c); return (<button key={c.id} onClick={() => setSelectedConvo(c)} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-page border-b border-surface-50"><div className="flex h-9 w-9 items-center justify-center rounded-full accent-bg text-sm font-semibold accent-color">{(o?.name||o?.email||"?").charAt(0).toUpperCase()}</div><div className="flex-1 min-w-0"><p className="text-sm font-medium text-heading truncate">{o?.name||o?.email}</p>{c.messages[0] && <p className="text-xs text-body truncate">{c.messages[0].content}</p>}</div></button>); }))}</div>
          ) : (<><div className="flex-1 overflow-y-auto px-3 py-3">{messagesLoading ? <div className="flex items-center justify-center py-12"><div className="h-5 w-5 animate-spin rounded-full border-2 border-themed border-t-[var(--accent)]" /></div> : messages.length === 0 ? <p className="text-center text-xs text-body py-12">No messages yet</p> : <div className="space-y-3">{messages.map((m) => { const isOwn = m.user.id === currentUserId; return (<div key={m.id} className={cn("flex", isOwn ? "justify-end" : "justify-start")}><div className={cn("max-w-[75%] rounded-2xl px-3.5 py-2", isOwn ? "bg-brand-600 text-white" : "bg-hover text-heading")}><p className="text-[13px] whitespace-pre-wrap break-words">{m.content}</p><p className={cn("text-[10px] mt-0.5", isOwn ? "text-[var(--accent)]" : "text-body")}>{new Date(m.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></div></div>); })}<div ref={messagesEndRef} /></div>}</div><div className="border-t border-themed-subtle px-3 py-2.5"><div className="flex items-center gap-2"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Type a message..." className="flex-1 rounded-xl border border-themed bg-page px-3.5 py-2 text-sm text-heading placeholder-surface-400 outline-none focus:border-brand-400" /><button onClick={handleSend} disabled={!input.trim()||isPending} className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-600 text-white hover:bg-brand-700 disabled:opacity-50"><Send className="h-4 w-4" /></button></div></div></>)}
        </div>
      )}
    </>
  );
