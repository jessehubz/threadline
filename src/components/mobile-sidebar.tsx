...
export function MobileSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="fixed inset-y-0 left-0 w-[280px] bg-card shadow-2xl flex flex-col">
        {/* Header */}
        <div className="flex h-14 items-center justify-between px-5 border-b border-themed-subtle">
          <a href="/dashboard" className="flex items-center gap-0">
            <span className="text-[20px] font-bold tracking-tight text-heading font-[var(--font-logo)]">Thread</span>
            <span className="text-[20px] font-bold tracking-tight accent-color">line</span>
          </a>
          <button onClick={onClose} className="rounded-lg p-1.5 text-dim hover:bg-hover" aria-label="Close menu">
            <X className="h-5 w-5" />
          </button>
        </div>
