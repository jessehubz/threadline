...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  NOT_STARTED: { label: "Not Started", color: "text-[#6B7280]", bg: "bg-surface-100" },
  ...
  if (projects.length === 0) {
    return (
      <div className="rounded-2xl border border-surface-200/60 bg-white p-12 text-center">
        ...
        <p className="text-sm font-medium text-[#1A1A1A]">No projects to overview</p>
        <p className="text-xs text-[#6B7280] mt-1">You need to be a Head or Co-Head of a project to see its overview.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <button
            ...
            className={cn(
              ...
              selectedId === p.id
                ...
                : "bg-white border border-surface-200 text-[#6B7280] hover:border-brand-300 hover:text-brand-600"
            )}
          >
            {p.name}
          </button>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-surface-200/60 bg-white p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-semibold text-[#9CA3AF] uppercase">Total</span>
          </div>
        ...
        <div className="rounded-xl border border-surface-200/60 bg-white p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-semibold text-[#9CA3AF] uppercase">Done</span>
          </div>
          <p className="text-[24px] font-bold text-[#1A1A1A]">{completed}</p>
        </div>
        <div className="rounded-xl border border-surface-200/60 bg-white p-4">
          <div className="flex items-center gap-2 mb-1">
            <Clock className="h-4 w-4 text-blue-500" />
            <span className="text-[11px] font-semibold text-[#9CA3AF] uppercase">Active</span>
          </div>
          <p className="text-[24px] font-bold text-[#1A1A1A]">{inProgress}</p>
        </div>
        <div className="rounded-xl border border-surface-200/60 bg-white p-4">
          <div className="flex items-center gap-2 mb-1">
            <AlertCircle className="h-4 w-4 text-red-500" />
            <span className="text-[11px] font-semibold text-[#9CA3AF] uppercase">Blocked</span>
          </div>
          <p className="text-[24px] font-bold text-[#1A1A1A]">{blocked}</p>
        </div>
      </div>
      ...
      {totalTasks > 0 && (
        <div className="rounded-xl border border-surface-200/60 bg-white p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[13px] font-semibold text-brand-600">{Math.round((completed / totalTasks) * 100)}%</span>
          </div>
      ...
      <div className="rounded-xl border border-surface-200/60 bg-white p-5">
        <h3 className="text-[14px] font-semibold text-[#1A1A1A] mb-4">Team Progress</h3>
        <div className="space-y-3">
          {memberStats.map((m) => (
            <div key={m.id} className="flex items-center gap-3 py-2 border-b border-surface-50 last:border-0">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-100 text-xs font-semibold text-brand-700 flex-shrink-0">
                {(m.user.name || m.user.email).charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-[#1A1A1A] truncate">{m.user.name || m.user.email}</p>
                <p className="text-[11px] text-[#9CA3AF]">{m.role} · {m.completedTasks}/{m.totalTasks} done</p>
              </div>
              <div className="w-20">
          ...
          {memberStats.length === 0 && <p className="text-[13px] text-[#9CA3AF] text-center py-4">No team members</p>}
        </div>
      </div>

      {/* Task list */}
      <div className="rounded-xl border border-surface-200/60 bg-white p-5">
        <h3 className="text-[14px] font-semibold text-[#1A1A1A] mb-4">All Tasks</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="border-b border-surface-100">
                <th className="pb-2 pr-4 font-medium text-[#6B7280]">Task</th>
                <th className="pb-2 pr-4 font-medium text-[#6B7280]">Assigned To</th>
                <th className="pb-2 pr-4 font-medium text-[#6B7280]">Status</th>
                <th className="pb-2 font-medium text-[#6B7280]">Due</th>
              </tr>
            </thead>
            <tbody>
              {tasks.map((task) => {
                ...
                return (
                  <tr key={task.id} className="border-b border-surface-50 hover:bg-surface-50/50">
                    <td className="py-2.5 pr-4 font-medium text-[#1A1A1A]">{task.title}</td>
                    ...
                    <td className={cn("py-2.5 text-[12px]", isOverdue ? "text-red-600 font-medium" : "text-[#6B7280]")}>
          ...
          {tasks.length === 0 && <p className="py-6 text-center text-[13px] text-[#9CA3AF]">No tasks in this project</p>}
        </div>
      </div>
